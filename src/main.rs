...
fn efi_main(image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    let vram_adder = efi_graphics_output_protocol.mode.frame_buffer_base;
    let vram_byte_size = efi_graphics_output_protocol.mode.frame_buffer_size;
    let vram = unsafe {
        slice::from_raw_parts_mut(
            vram_adder as *mut u32,
    ...
    */
    let vw = vram.width;
    let vh = vram.height;
    fill_rect(&mut vram, 0x000000, 0, 0, vw, vh).expect("fill_rect failed"); // 描画範囲を黒で塗りつぶし
    /*
...
fn darw_test_pattern<T: Bitmap>(buf: &mut T) {
    let w = 128;
...
// use core::{mem::offset_of, panic::PanicInfo};
